...
const User = ({ src, name, text }) => {
  const [showOptions, setShowOptions] = useState(false)
   ...
   const toggleShowOptionsHandler = () => setShowOptions(prevState => !prevState)
  ...
  return (
    <div className='w-full bg-lightGrey py-1 px-2 text-darkGrey grid grid-rows-2 grid-cols-1 md:grid-cols-2 md:grid-rows-1 items-center'>
      {showOptions && <div style={{position: 'absolute', marginLeft: '32rem', marginTop: '2.5rem', zIndex: '2000'}} className=''><OptionContainer /> </div>}
      ...
      <div className='flex items-center justify-end'>
        <div>{text}</div>
        <div>
          <div className='cursor-pointer mx-3' onClick={toggleShowOptionsHandler}>{<Option />}</div>
